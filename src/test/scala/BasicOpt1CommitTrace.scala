...
class BasicOpt1CommitTrace extends AnyFlatSpec with ChiselScalatestTester {
  "Core" should "dump basicopt1 commit trace" in {
    val outPath = "tmp_dut_basicopt1_trace.txt"
    ...
    try {
      test(new Core(initFile = "src/test/resources/basicopt1.data", memSize = 131072, memDelay = 4)) { c =>
        c.clock.setTimeout(0)
        var cycles = 0
        var commits = 0
        val maxCycles = intProp("cpu.traceMaxCycles", 300000)
